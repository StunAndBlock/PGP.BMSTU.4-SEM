...
int resize(unsigned w, unsigned h) {
    ...
    if ((bak.width == w) && (bak.height == h)) return (0); /* remove window */
    bak.width = w;
    bak.height = h;
...
int rescale(unsigned w, unsigned h) {
    ...
    x = w / NUNIT;
    y = h / NUNIT;
    if ((scale.x == x) && (scale.y == y)) return (0); /* small resize without change scale */
    scale.x = x;
    scale.y = y;
...
int reconf(unsigned w, unsigned h) {
    if (resize(w, h) == 0) return (0);
    if (rescale(w, h) != 0) rebuild();
    return (NFACE);
} /* reconf */
...
int zotone(int x, int y) {
    ...
    past.x = scale.x;
    past.y = scale.y;           /* Store scale */
    for (f = 0; f < NFACE; f++) /* find face with (x,y) inside */
        if (XPointInRegion(face[f].regi, x, y) == True) break;
