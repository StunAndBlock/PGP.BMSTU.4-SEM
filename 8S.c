    ...
    case ButtonRelease:
      ...
      if ((flag == 2 || flag == 3 || flag == 4) && newEnv->rectanglesCount) {
        ...
        for (unsigned short i = 0; i < newEnv->rectanglesCount - 1; i++) {
          ...
          XFillRectangle(newEnv->dpy, newEnv->rectangles[i].winBox,
                         newEnv->gc[3], 2, 2, xwa.width - 4, xwa.height - 4);
        }
        ...
        XFillRectangle(newEnv->dpy,
                       newEnv->rectangles[newEnv->rectanglesCount - 1].winBox,
                       newEnv->gc[2], 2, 2, xwa.width - 2, xwa.height - 2);
        for (int i = 0; i < newEnv->rectanglesCount; i++) {
          fprintf(stderr, "%d:{%d*2} ", i, newEnv->rectangles[i].K);
        }
        fprintf(stderr, "\n");
      }
      break;
