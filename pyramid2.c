...
Window canvas(Display* dpy) {
    ...
    attr.event_mask = (ButtonPressMask | KeyPressMask | ExposureMask | StructureNotifyMask |
    ...
    x = w / 2;
    y = h / 2;
    win = XCreateWindow(dpy, DefaultRootWindow(dpy), x, y, w, h, 1, DefaultDepth(dpy, scr), InputOutput,
    ...
    hint.max_width = 2 * w;
    hint.max_height = 2 * h; /* window max size */
    hint.x = x;
    hint.y = y;
...
int regraph(Display* dpy, Window win, GC gc, int NoFillFace) {
    ...
    for (i = NoFillFace; i < NFACE; i++) { /* redraw all faces */
        ...
        XFillPolygon(dpy, win, gc, face[i].top, face[i].Cn, Convex, CoordModeOrigin);
    }                                         /* for face */
